...
wsh = gc.open("Hospital").get_worksheet(0)

sh = gc.open("Hospital").get_worksheet(1)

sh2 = gc.open("Hospital").get_worksheet(2)
...
def write():
    ...
    for i in range(int(in1)):
        ...
        def input_y(a):
            ...
            while int(a) < 1930 or int(a) > 2020:
                print("Unos nije u redu!")
                a = input("Unesite godinu: ")
                continue
        ...
        # input for patients names with title()
        name = raw_input("Unesite ime i prezime pacijenta: ").title()
        ...
        while len(name.split()) != 2:
            ...
            name = input("Unesite ime i prezime pacijenta: ").title()
            continue
        ...
        if name in name_list:
            ...
            print(df[df["IME I PREZIME"] == name])
        else:
            pass
        ...
        # user warning for wrong input
        try:
            d2 = datetime.time(hour=int(h[0:2]), minute=int(h[2:4]))
            d4 = datetime.time(hour=int(h1[0:2]), minute=int(h1[2:4]))
            d5 = str(datetime.timedelta(hours=(d4.hour - d2.hour), minutes=(d4.minute - d2.minute)))
        except ValueError:
        ...
        row = [name, form, form2, patient_year2, str(d2)[:-3], str(d4)[:-3], str(d6)[:-3]]
        index = 2
